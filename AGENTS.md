...
- Two windows: main window + hidden danmu overlay (`frontend/danmu-overlay.html`, vite multi-entry build; second HTML/JS entry). Note: vite names output HTML after the source file path, not the input key — keep the filename in sync with `get_html_path()` in `main.py`. Overlay window is created hidden in `main.py` and shown via `toggle_danmu_overlay`; danmu messages are pushed to it too (`window_service.send_to_window`). Overlay uses its own `OverlayApiProxy` js_api and Linux drag via `startSystemMove` (Wayland: `window.move` is a no-op).
